...
public class StoreController {
	...
	public ModelAndView storeDetail(@ModelAttribute Item item, BindingResult bindingResult,
		...
		model.addAttribute("store_name",sn);
		return mv;
	}
	@RequestMapping(value="/buyItemFromStore/{id}",method= RequestMethod.POST)
	public RedirectView insertData(@ModelAttribute Buy buy, BindingResult bindingResult,
		...
		if(temp != null) {
			...
			if(sisa < 0) {
				...
				return new RedirectView("/startOrderFormStore/{id}");
			}
				...
				return new RedirectView("/startOrderFromStore/{id}");
			}
				...
				return new RedirectView("/storeDetail/"+selectedItem.getStore_name());
			}
		}
		mv.addObject("items",items);
		return new RedirectView("/item");
	}
	@GetMapping(value="/startOrderFromStore/{id}")
	public ModelAndView startOrder(@ModelAttribute Item item, BindingResult bindingResult,
		...
		Buy buy = new Buy();
		ModelAndView mv = new ModelAndView("buyItemFromStore");
	...
	@GetMapping(value="/cancelBuyFromStore/{id}")
	public RedirectView cancelBuy(@ModelAttribute Item item, BindingResult bindingResult,
		...
		Item selectedItem = mongoTemplate.findOne(query, Item.class);
